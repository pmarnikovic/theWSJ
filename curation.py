...
from bs4 import BeautifulSoup  # Added for parsing embedded images in summaries--- CONFIGURATION ---Configure the Gemini API clienttry:
    ...
    print(f"ERROR: {e}")
    exit(1)List of RSS feeds to parse for AI news (enhanced with more business/M&A focused sources)RSS_FEEDS = [
...
MAX_TECHNICAL_ARTICLES = 3Style options for random assignmentSTYLE_CLASSES = ['style1', 'style2', 'style3', 'style4']Source mapping for friendly namesSOURCE_MAP = {
    'theverge': 'The Verge',
...
}AI relevance keywords (to filter non-AI stories)AI_KEYWORDS = [
...
]M&A keywords for boostMA_KEYWORDS = ['merger', 'acquisition', 'm&a', 'buyout', 'investment', 'funding round', 'venture capital', 'acquired', 'merged']Key people moves keywordsPEOPLE_KEYWORDS = ['hire', 'hires', 'leaves', 'joins', 'promoted', 'ceo', 'executive', 'cfo', 'cto', 'resigns', 'appointment']Breakthrough/excitement keywordsBREAKTHROUGH_KEYWORDS = ['breakthrough', 'revolution', 'game-changing', 'transformative', 'new model', 'agi', 'hardware advance', 'software breakthrough', 'innovation', 'first-ever', 'unveils', 'launches']Key AI people for extra boost (curated list)KEY_AI_PEOPLE = [
...
]Updated Fortune tiers (from 2024 data)fortune_top_20 = [
...
fortune_101_to_500 = []  # Can add if needed for minor boostsPopular AI companies (expanded)popular_ai_companies = [
...
]--- FUNCTIONS ---def get_articles_from_rss():
    ...
    return articlesdef get_articles_from_arxiv():
    ...
    return articlesdef is_ai_related(title, summary):
    """Check if the article is AI-related using keywords."""
    text = (title + " " + summary).lower()
    return any(kw in text for kw in AI_KEYWORDS)def get_headline_and_score(title, summary):
    ...
    Third, if this seems technical/academic or a breakthrough in AI HW/SW, rate its importance/impact on a scale of 1-10 (higher for game-changers); otherwise, return 0.Respond with the score, the headline, and tech_importance separated by pipe characters (|). For example: 18|Microsoft unleashes AI revolution that could change everything!|8
...
try:
    ...
    return fallback_headline, 5, 0def get_related_image_url(headline):
    ...
    return "https://via.placeholder.com/600x300?text=AI+Headline+Image"  # Fallbackdef get_boost(title, summary, source):
    """Calculate boost based on company mentions, M&A, people moves, breakthroughs."""
    text = (title + " " + summary).lower()
    boost = 0# Boost for popular AI companies
...
return min(max(boost, 0), 100)  # Cap boost, no negative--- MAIN EXECUTION ---if name == "main":
    print("Starting content generation for Exhaustive AI...")# 1. Fetch and deduplicate articles
